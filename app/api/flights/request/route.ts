...
export async function POST(request: Request) {
  ...
  const response = webhookUrl ? await fetch(`${webhookUrl}?wait=true&with_components=true`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: "Aer Infinity Route Planning", ...payload }) }) : botToken ? await fetch(`https://discord.com/api/v10/channels/1358547247016444223/messages`, { method: "POST", headers: { Authorization: `Bot ${botToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ content: "New Aer Infinity route request", ...payload }) }) : new Response(null, { status: 503 });
